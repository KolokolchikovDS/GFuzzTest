...
void __sanitizer_weak_hook_strcmp(void *caller_pc, const char *s1,
  ...
  // No-op on Windows: InternalStrnlen2() scans both strings until a NUL, which
  // is unsafe when strcmp() is called with string_view-style, non-null-
  // terminated buffers. ASan then reports a global-buffer-overflow. Dropping
  // this hook only loses string-compare value guidance at these callsites.
  (void)caller_pc; (void)s1; (void)s2; (void)result;
}
...
void __sanitizer_weak_hook_strstr(void *called_pc, const char *s1,
  ...
  // No-op on Windows: strlen(s2) is unsafe when strstr() is called with a
  // non-null-terminated needle (string_view-style), tripping ASan. Only loses
  // the mutation-dictionary entry for these callsites.
  (void)called_pc; (void)s1; (void)s2; (void)result;
}

ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY
void __sanitizer_weak_hook_strcasestr(void *called_pc, const char *s1,
                                      const char *s2, char *result) {
  // No-op on Windows; see __sanitizer_weak_hook_strstr.
  (void)called_pc; (void)s1; (void)s2; (void)result;
}
